(() => {
    ...
    da.init();
    if (document.querySelector(".header__timer")) {
        ...
        const endTime = (new Date).getTime() + 2 * 24 * 60 * 60 * 1e3;
        startTimer(endTime);
    }
    if (document.querySelector(".scroll-block")) {
        var textBlock = document.querySelector(".scroll-block__text");
        for (var i = 0; i < 4; i++) {
            var copy = textBlock.cloneNode(true);
            document.querySelector(".scroll-block").appendChild(copy);
        }
    }
    window["FLS"] = false;
})();